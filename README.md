...
基于 OpenAI Whisper模型开发构建，配备**专业术语字典**和**自主优化引擎**，不仅能根据专业术语字典库进行命中纠正，还能**根据你的从事的具体方向，可持续自主学习优化**

🤓 **功能说明**：
- 🧠 **社区字典**：Github共建共享，协助提示词动态构建与兜底命中纠错双重保障
...
```mermaid
graph LR
    A[用户说话] --> B[根据用户画像动态构建prompt并喂给Whisper进行推理转文字]
    B --> C[兜底字典命中修正]
    C --> D[检测用户高频词，本地记录用户数据]
    D --> E[根据本地数据形成用户特定术语库，持续描绘用户画像]
    E --> F[下次启动时再次动态构建Whisper的提示词，优化下次识别]
    F --> B
```
...
| 识别"Redis"需字典兜底映射进行纠正 | 改变模型特定方向输出文本的分布概率，直接识别"Redis"！ | 🎯 无需再走字典兜底命中               |
| 知道你是开发者，但是不知道你的具体方向  | **根据你说的高频词，本地分析你的具体方向**        | 🧠 无需进行大模型微调，实现可持久化的轻量级自主学习 |
...
cd CodeWhisper
2.创建虚拟环境（如果你是Pycharm打开，按照它的提示创建即可）
python -m venv .venv
...
- **Python 版本**：3.9+
- **网络**：首次运行需要下载 Whisper 模型（100MB-3GB，取决于选择的模型,medium初次使用需要下载1.4G的模型,注意你的流量哦）
...
A:
系统通过你的高频术语进行技术栈分析：
- 经常说 `Dubbo、Redis、Kafka` → 基本为后端开发者
- 经常说 `React、Vue、Webpack` → 基本为前端开发者

自动优化提示词，让 Whisper 优先识别你领域的术语
